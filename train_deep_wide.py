...
def valid_epoch(model, loader, criterion, device, epoch):
    ...
    shahed_f1 = threshold_info["shahed"]["f1"]
    shahed_threshold = threshold_info["shahed"]["threshold"]

    return val_loss / val_total, f1, precision, recall, shahed_f1, shahed_threshold
...
def main():
    ...
    model = ShahedNetDeepWide(n_classes=len(label2idx)).to(device)
    ...
    scheduler = CyclicLR(
        ...
        base_lr=1e-6,
        max_lr=1e-4,
        step_size_up=2600,  # ~10 епох (10 × 260 ітерацій)
        mode='triangular2',
        cycle_momentum=False
    )

    best_shahed_f1 = 0.0
    best_shahed_threshold = 0.5

    for epoch in range(epochs):
        logger.info(f"Епоха {epoch + 1}/{epochs}")
        train_loss, train_f1, train_prec, train_rec = train_epoch(model, train_loader, criterion, optimizer, scheduler, device, epoch)
        val_loss, val_f1, val_prec, val_rec, shahed_f1, shahed_thr = valid_epoch(model, valid_loader, criterion, device, epoch)
        ...
        logger.info(
            ...
            f"Shahed F1: {shahed_f1:.4f} (Threshold={shahed_thr:.2f})"
        )

        if shahed_f1 > best_shahed_f1:
            best_shahed_f1 = shahed_f1
            best_shahed_threshold = shahed_thr
            torch.save(model.state_dict(), "saved_models/shahednet_best.pth")
            logger.info(f"Збережено кращу модель (shahed F1={shahed_f1:.4f}) на епосі {epoch + 1}")
