...
def record_and_predict(model, device, input_rate=48000, target_rate=16000, segment_duration=1.0, threshold=0.8):
...
def main():
    ...
    model.load_state_dict(torch.load("saved_models/shahednet_best_deep_wide.pth", map_location=device))
    model.to(device)
